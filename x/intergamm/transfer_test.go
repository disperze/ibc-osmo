...
// IBC Transfer tests taken from https://github.com/cosmos/ibc-go/blob/v2.0.3/modules/apps/transfer/transfer_test.go
// but use simApp from this repo
...
func (suite *TransferTestSuite) SetupTest() {
	// Configure app with swap middleware
...
func (suite *TransferTestSuite) TestHandleMsgTransfer() {
	...
	balance := suite.GetSimApp(suite.chainB).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	...
	balance = suite.GetSimApp(suite.chainC).BankKeeper.GetBalance(suite.chainC.GetContext(), suite.chainC.SenderAccount.GetAddress(), coinSentFromBToC.Denom)

	// check that the balance is updated on chainC
	suite.Require().Equal(coinSentFromBToC, balance)

	// check that balance on chain B is empty
	balance = suite.GetSimApp(suite.chainB).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), coinSentFromBToC.Denom)
	...
	balance = suite.GetSimApp(suite.chainB).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), coinSentFromAToB.Denom)
	...
	balance = suite.GetSimApp(suite.chainB).BankKeeper.GetBalance(suite.chainB.GetContext(), escrowAddress, sdk.DefaultBondDenom)
	suite.Require().Equal(sdk.NewCoin(sdk.DefaultBondDenom, sdk.ZeroInt()), balance)

	// check that balance on chain B is empty
	balance = suite.GetSimApp(suite.chainC).BankKeeper.GetBalance(suite.chainC.GetContext(), suite.chainC.SenderAccount.GetAddress(), voucherDenomTrace.IBCDenom())
	suite.Require().Zero(balance.Amount.Int64())
}

func (suite *TransferTestSuite) GetSimApp(chain *ibctesting.TestChain) *simapp.SimApp {
